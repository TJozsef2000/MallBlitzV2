...
test("interact with web components using Shadow DOM", async ({ page }) => {
	...
	await page.locator("my-app").locator("my-sidebar").getByRole("link", { name: "Dashboard" }).click();
	...
	const eventPromise = page.evaluate(() => {
		return new Promise<{ detail: unknown }>((resolve) => {
			document
				...
				.addEventListener("color-change", (e: Event) => resolve({ detail: (e as CustomEvent).detail }), {
					once: true,
				});
		});
	});
